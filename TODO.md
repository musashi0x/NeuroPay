...
- [x] Run the revoke and record its hash. _(Done 2026-08-20T04:40:42Z, tx `0x5818a511ea7d9b03798f9396ae46afdb3408f9e096c8b7c7bc05c888300d3a23`, receipt `success` in block 126120934, 137,195 gas — confirmed against the chain, not just the runbook entry. Note the keystore keeps a revoked key **registered**: re-granting that same session key later fails with `KeyStore: key already registered`, so a replacement session needs a new key rather than a re-grant. That is a real operational constraint and cost one grant attempt to discover.)_
...
- [ ] Run the full chain-97 loop: open stream, accrue, receive 402, sign, verify, deliver, submit settlement, confirm settlement, and reconcile the ledger. _(**Every prerequisite is now in place.** Wallet holds 1,000,000 npUSD; Permit2 approved unlimited; session granted 2026-08-20T12:46Z (tx `0x46c23977…`, `success`, block 126185617) with a 10 npUSD/day cap scoped to the new token and a Permit2 `transferFrom` allowlist; rail re-provisioned for npUSD (`0x3130f83f…`, `0x3bfb430f…`, both `success`); `checkSessionAuthority` reads `active / isValidKey=true`; the record is persisted at `apps/api/.data/session.json`, which earlier grants never managed. Session expires 2026-08-21T12:45:46Z. What remains is running it: start the seller with `SETTLER_PRIVATE_KEY` set, then `pnpm --filter @neuro-pay/api demo:real`.)_
